"""
Этот файл содержит в себе класс для создания и описания БД,
а также класс для получения данных и манипуляции с БД.
"""
...
logging.basicConfig(level=logging.DEBUG,
                    filename=pathlib.Path.cwd() / 'project_logs.log', filemode='w')
...
class DBCreator:
    ...
    def create_tables(cls) -> None:
        ...
        blacklisted_employers, blacklisted_vacancies.
        """
        logging.debug(f'Started {currentframe().f_code.co_name} func')
...
class DBManager:
    ...
    @classmethod
    def save_employer_to_db(cls, comp_id: int, name: str, industry: str,
                            vac_count: int, url: str) -> None:
        """
        Добавить работодателя в список отслеживания.
        :param comp_id: ID работодателя.
        :param name: Название работодателя.
        :param industry: Отрасль деятельности.
        :param vac_count: Кол-во вакансий у работодателя.
    ...
    def blacklist_vacancy(cls, vac_id: int, blacklisted: bool = True) -> None:
        """
        Добавить вакансию в ЧС.
        :param vac_id: ID вакансии.
        :param blacklisted: Статус ЧС.
        """
        logging.debug(f'Started {currentframe().f_code.co_name} func')
        with db.connect(db_config) as conn:
            with conn.cursor() as cur:
    ...
    @classmethod
    def check_blacklisted_employer(cls, emp_id: int) -> bool:
        """
        Проверяет, есть ли работодатель в ЧС.
        :param emp_id: ID работодателя для проверки наличия в ЧС.
        :return: True если работодатель в ЧС.
        """
        logging.debug(f'Started {currentframe().f_code.co_name} func')
        with db.connect(db_config) as conn:
            with conn.cursor() as cur:
    ...
    def check_vacancy_in_db(cls, vac_id: int) -> bool:
        ...
        :return: True если вакансия уже в БД.
        """
        logging.debug(f'Started {currentframe().f_code.co_name} func')
        with db.connect(db_config) as conn:
            with conn.cursor() as cur:
